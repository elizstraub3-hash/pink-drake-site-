const DB = {
  ...
  updateProduct(id, data) {
    ...
    if (i !== -1) { list[i] = { ...list[i], ...data }; this.saveProducts(list); return list[i]; }
    return null;
  },
  ...
  getSales() {
    const d = localStorage.getItem(this.SALES_KEY);
  ...
  addSale(s) {
    ...
    list.unshift(s);
    localStorage.setItem(this.SALES_KEY, JSON.stringify(list));
    return s;
  },

  deleteSale(id) {
    localStorage.setItem(this.SALES_KEY, JSON.stringify(this.getSales().filter(s => s.id !== id)));
  },

  exportCSV(sales) {
    const cols = ['ID','Data','Cliente','Telefone','Produtos','Total','Tipo','Endereço','Canal','Obs'];
